...
function App() {
  ...
  // ✅ NOTIFICACIONES (1 SOLO SWITCH PARA TODO)
  ...
  // ✅ Detectar soporte y cargar preferencia de notifs
  useEffect(() => {
    const supported =
    ...
    const saved = localStorage.getItem("mdekot_notifs_enabled");
    if (saved === "true") setNotifsEnabled(true);
    if (saved === "false") setNotifsEnabled(false);
  }, []);
  ...
  // ✅ Toggle ON/OFF notificaciones (por ahora: pide permiso + guarda preferencia)
  const toggleNotifs = async () => {
    ...
    if (next) {
      try {
        ...
        if (perm !== "granted") {
          // si no acepta, no activamos
          localStorage.setItem("mdekot_notifs_enabled", "false");
          setNotifsEnabled(false);
          return;
        }

        // Guardar ON
        localStorage.setItem("mdekot_notifs_enabled", "true");
        setNotifsEnabled(true);

        // 🔜 En el siguiente paso (firebase.js + SW + functions):
        // aquí registraremos token FCM en Firestore para notifs con app cerrada.
      ...
      // aquí eliminaremos el token FCM de Firestore para desactivar push.
    }
  };
  ...
  return (
    <div style={{ ...styles.container, padding: isMobile ? "16px" : "40px" }}>
      <div style={styles.tabs}>
        ...
        {/* ✅ BOTÓN ON/OFF (vale para TODO) */}
        <button
          onClick={toggleNotifs}
          disabled={!notifSupported}
          style={{
            ...(notifsEnabled ? styles.notifOn : styles.notifOff),
            ...
            <div style={styles.modalOverlay}>
              <div style={{ ...styles.modal, maxWidth: "420px" }}>
                <h3 style={{ marginTop: 0 }}>
                  📌{" "}
                  {diaDetalleFecha
                    ? new Date(diaDetalleFecha + "T00:00:00").toLocaleDateString("es-ES", {
                        weekday: "long",
                        day: "2-digit",
                        month: "long"
                      })
                      ...
                      <div key={ev.id} style={{ ...styles.dayDetailRow, borderLeft: `10px solid ${colorEventoPorIndice(idx)}` }}>
                        <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", alignItems: "center" }}>
                          <div style={{ minWidth: 0 }}>
                            ...
                            {ev.notas ? (
                              <div style={{ opacity: 0.9, fontSize: "13px", textAlign: "left", marginTop: "6px" }}>{ev.notas}</div>
                            ) : null}
                          </div>

                          <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
                            <button onClick={() => abrirEditarEvento(ev)} style={styles.buttonEdit}>
                              ✏
                            </button>
                            <button onClick={() => setEventoAEliminar(ev)} style={styles.buttonDelete}>
                              🗑
                            </button>
                          </div>
                        </div>
...
const styles = {
  ...
  // ✅ nuevo estilo botón notifs
  notifOn: { background: "#22c55e", color: "#111827", padding: "10px 16px", border: "none", borderRadius: "999px", cursor: "pointer", fontWeight: 900 },
  notifOff: { background: "#1e293b", color: "white", padding: "10px 16px", border: "1px solid rgba(255,255,255,0.22)", borderRadius: "999px", cursor: "pointer", fontWeight: 900 },
  ...
  calDayNumber: { fontWeight: 900, opaimport { useEffect, useMemo, useState } from "react";
import {
  collection,
  addDoc,
  updateDoc,
...
export default App;
city: 0.9, fontSize: "13px", textAlign: "center", width: "100%" },
  calDayNumberToday: { color: "#22c55e" },
